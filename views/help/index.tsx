...
export default function Help() {
  ...
  const onSubmit = async (data: HelpFormData) => {
    ...
    try {
      ...
      await AsyncStorage.setItem("@Atos:pending_help_id", request.id);
  ...
  useEffect(() => {
    const checkActiveRequest = async () => {
      try {
        const savedId = await AsyncStorage.getItem("@Atos:pending_help_id");
        ...
        console.error("Erro na checagem de pedido ativo:", err);
        await AsyncStorage.removeItem("@Atos:pending_help_id");
        setIsChecking(false);
      }
    };
  ...
  if (isChecking) {
    return (
      <Center className="flex-1 bg-atosBlue">
        <VStack space="md">
          <Spinner size="large" color="$atosAmber" />
          <Text className="text-white">Verificando conexões...</Text>
        </VStack>
      </Center>
    );
  }
              ...
              <Controller
                ...
                render={({ field: { onChange, onBlur, value } }) => (
                  <HStack className="items-center">
                    <User
                      size={24}
                      color={errors.name ? "#ef4444" : "#F59E0B"}
                      className="mr-3"
                    />
                    <Input
                      variant="outline"
                      size="md"
                      className="mr-2flex-1 border-0 bg-transparent p-0 h-auto"
              ...
              <Controller
                ...
                render={({ field: { onChange, onBlur, value } }) => (
                  <HStack className="items-start flex flex-1">
                    <Heart
                      size={24}
                      color={errors.message ? "#ef4444" : "#F59E0B"}
                      className="mr-3 mt-1"
                      fill={errors.message ? "#ef4444" : "#F59E0B"}
                    />
                    <Textarea
                      size="md"
                      className="flex-1 border-0 ml-2 bg-transparent p-0 h-full w-full"
